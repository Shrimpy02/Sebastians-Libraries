...
public:
	...
	void testHeap()
	{
		...
		double testHeap[heapMaxSize] = { 10, 5, 3, 2, 4 };
		int size = 5;
		int newValue = 15;
...
private:
	...
	void heapify(double _arr[], int _size, int _index)
	{
		...
		if(_arr[parent] > 0)
		{
			// checks if the new value is greater than the parents value
			if(_arr[_index] > _arr[parent])
			{
				// swaps the parent and the new value so the greater value becomes the highest. 
				std::swap(_arr[_index], _arr[parent]);
